...
themselves. Not only that, same goes for updates.s
...
auto complete JSX for me. I would have to manually type in JSX Component tags, which became frustrating and time consuming.
...
I used figma to create the design and build a prototype of my app, for reference.
...
the database whenever it starts for the first time. It took a lot of manual tinkering to get the database intialized.
...
I then came across React 'Context API'. That allowed me to initialise the database once and use it everywhere
to run my CRUD operations easily.

#### 3. Sliding left to reveal delete and edit options

I spent a significant amount of time trying to figure out ways to make sliding gestures work. To make items show
edit and delete option when slid to left. Finally, managed to made it work, it can still be better. But for now, I am happy
with it.
